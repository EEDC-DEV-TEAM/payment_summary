        ...
        if (contacts[i].district === summary[j].district) {
          ...
          if (summary[j].transactions.length > 1) {
            message =
              ...
              '\n' +
              'Payments: ' +
              summary[j].transactions[0].count +
              '\n' +
              'Total: N' +
              numberWithCommas(summary[j].transactions[0].total) +
              '\n----\n' +
              summary[j].transactions[1].plan.toUpperCase() +
              '\n' +
              'Payments: ' +
              summary[j].transactions[1].count +
              '\n' +
              'Total: N' +
              numberWithCommas(summary[j].transactions[1].total) +
              '\n----\n' +
              'Day Cumulative: N' +
              numberWithCommas(summary[j].daytotal) +
              '\n----\n' +
              'MTD: N' +
              numberWithCommas(summary[j].mtd)
          } else {
            message =
              summary[j].district +
              ' collections as at:\n' +
